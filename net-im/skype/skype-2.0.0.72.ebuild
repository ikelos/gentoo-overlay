...
pkg_setup() {
	if ! use qt-static ; then
		if has_version '<x11-libs/qt-4.4.0_alpha:4' &&	! built_with_use ">=x11-libs/qt-4.0" accessibility dbus ; then
		...
		elif has_version '>=x11-libs/qt-4.4.0_alpha:4' && ! built_with_use ">=x11-libs/qt-gui-4.4.0:4" accessibility dbus ; then
                        eerror "Rebuild qt-gui-4 with USE=\"accessibility dbus\" or set USE=qt-static"
                        die "USE=\"-qt-static\" only works with qt-gui-4 built with USE=\"accessibility dbus\"."
		fi
	fi
}
